...
class btree {
    ...
    public:
        ...
        void splitChild(node *parent, node *leftNode) {
            ...
            // if leftNode is not a leaf, copy the second half child pointer of leafNode to rightNode
        ...
        void InOrderTraversal(node *root, ofstream &output) {
            int i = 0;
            if (root != nullptr) {
                while (i < root->size) {
                    if (!root->leaf) {
                        InOrderTraversal(root->childptr[i], output);
                    }
                    output << root->keys[i] << " ";
                    i++;
                }
                // print the last pointer attached to the last child
                if (!root->leaf) {
                    InOrderTraversal(root->childptr[i], output);
                }
        ...
        void printLevel(node *root, int level, ofstream &output) {
            if (level == 0) {
                output << "empty";
                return;
            }
            if (level > getHeight(root, 1)) {
                output << "empty";
                return;
            }
            if (level > 1 && root->leaf) {
                output << "empty";
                return;
            }
            if (level == 1) {
                ...
                for (int i = 0; i <= root->size; i++) {
                    printLevel(root->childptr[i], level - 1, output);
                }
            }
        }
        ...
        int getHeight(node *root, int h) {
            ...
            return getHeight(root->childptr[0], h + 1);
        }
};
